 ...
 public:
  ...
  void print_verilog(std::ostream& os, size_t& unconn_count, int depth = 0) override {
    ...
    if (mod_name.find("dff") == 0) {
      std::transform(mod_name.begin(), mod_name.end(), mod_name.begin(), ::toupper);
    }
...
class NetlistWriterVisitor : public NetlistVisitor {
 public:                                          // Public interface
  NetlistWriterVisitor(std::ostream& verilog_os,  ///< Output stream for verilog netlist
                       ...
                       struct t_analysis_opts opts)
      : verilog_os_(verilog_os), blif_os_(blif_os), sdf_os_(sdf_os), delay_calc_(delay_calc), opts_(opts) {
 ...
 protected:
  ...
  void print_verilog(int depth = 0) {
...
#ifdef ENABLE_VERIFIC
    if (isNestEncrypted) formating_encrypt_file(1);
#endif
    ...
    verilog_os_ << "\n";
#ifdef ENABLE_VERIFIC
    if (isNestEncrypted) formating_encrypt_file(2);
#endif
 ...
 protected:
  ...
  std::shared_ptr<Instance> make_latch_instance(const t_pb* atom) {
    ...
    std::string input_net = make_inst_wire(input_atom_net_id, find_tnode(atom, input_cluster_pin_idx),
                                           inst_name, PortType::INPUT, 0, 0);
    ...
    std::string output_net = make_inst_wire(output_atom_net_id, find_tnode(atom, output_cluster_pin_idx),
