...
BlifAllocCallback::BlifAllocCallback(e_circuit_format blif_format, AtomNetlist& main_netlist,
...
void BlifAllocCallback::names(std::vector<std::string> nets, std::vector<std::vector<blifparse::LogicValue>> so_cover) {
...
void BlifAllocCallback::latch(std::string input, std::string output, blifparse::LatchType type, std::string control, blifparse::LogicValue init) {
...
void BlifAllocCallback::subckt(std::string subckt_model, std::vector<std::string> ports, std::vector<std::string> nets) {
...
//BLIF Extensions
void BlifAllocCallback::conn(std::string src, std::string dst) {
...
void BlifAllocCallback::param(std::string name, std::string value) {
    if (blif_format_ != e_circuit_format::EBLIF) {
...
void BlifAllocCallback::parse_error(const int curr_lineno, const std::string& near_text, const std::string& msg) {
...
const t_model_ports* BlifAllocCallback::find_model_port(const t_model* blk_model, std::string port_name) {
