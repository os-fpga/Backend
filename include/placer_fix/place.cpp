...
static double cong_matrix[400][400];
static double cong_matrix_new[400][400];

static float congestion_tradeoff = 1.0;
...
static void get_non_updateable_bb_old(ClusterNetId net_id,
                                  t_bb& bb_coord_new);
...
static void get_cong_matrix(ClusterNetId net_id, const t_bb& bb_ptr);
...
static void get_bb_from_scratch_old(ClusterNetId net_id,
                                t_bb& coords,
                                t_bb& num_on_edges);
...
void try_place(const Netlist<>& net_list,
    ...
    if (skip_anneal == false) {
        ...
        do {
            ...
            if (get_cong_cost(placer_opts.congestion_tradeoff)<=20 && congest_flag==false
                                    && placer_opts.place_algorithm == CONGESTION_AWARE_PLACE) {
                congest_flag = true;
                congestion_tradeoff = 1.0;
...
static double get_total_cost(t_placer_costs* costs, const t_placer_opts& placer_opts, const t_noc_opts& noc_opts) {
    ...
    if (placer_opts.place_algorithm == BOUNDING_BOX_PLACE) {
        ...
        total_cost = (1-congestion_tradeoff) * (costs->cong_cost * costs->cong_cost_norm)
                     + ((1 - placer_opts.timing_tradeoff) * (costs->bb_cost * costs->bb_cost_norm)
...
static double comp_bb_cost(e_cost_methods method, const t_place_algorithm& place_algorithm) {
    ...
    for (auto net_id : cluster_ctx.clb_nlist.nets()) {       /* for each net ... */
        if (!cluster_ctx.clb_nlist.net_is_ignored(net_id)) { /* Do only if not ignored. */
            ...
            if (cluster_ctx.clb_nlist.net_sinks(net_id).size() >= SMALL_NET
                && method == NORMAL) {
                get_bb_from_scratch_old(net_id, place_move_ctx.bb_coords[net_id],
                                        place_move_ctx.bb_num_on_edges[net_id]);
            } else {
                get_non_updateable_bb_old(net_id,
...
static void get_bb_from_scratch_old(ClusterNetId net_id, t_bb& coords, t_bb& num_on_edges) {
...
static void update_cong_matrix(ClusterNetId net_id, t_bb* bb_ptr_old, t_bb* bb_ptr_new) {
    ...
    for (int i = bb_ptr_old->xmin; i < bb_ptr_old->xmax; i++) {
        for (int j = bb_ptr_old->ymin; j < bb_ptr_old->ymax; j++) {
            auto wl = get_net_wirelength_estimate(net_id, *bb_ptr_old);
            cong_matrix_new[i][j] -= wl / double((bb_ptr_old->xmax - bb_ptr_old->xmin + 1)*(bb_ptr_old->ymax - bb_ptr_old->ymin + 1));
        }
    }
    for (int i = bb_ptr_new->xmin; i < bb_ptr_new->xmax; i++) {
        for (int j = bb_ptr_new->ymin; j < bb_ptr_new->ymax; j++) {
            auto wl = get_net_wirelength_estimate(net_id, *bb_ptr_new);
            cong_matrix_new[i][j] += wl / double((bb_ptr_new->xmax - bb_ptr_new->xmin + 1)*(bb_ptr_new->ymax - bb_ptr_new->ymin + 1));
        }
    }
}

static void get_cong_matrix(ClusterNetId net_id, const t_bb& bbptr) {
    /* Finds the cost due to one net by looking at its coordinate bounding  *
     * box.                                                                 */
    // auto& cluster_ctx = g_vpr_ctx.clustering();
    ...
    for (int i = bbptr.xmin; i < bbptr.xmax; i++) {
        for (int j = bbptr.ymin; j < bbptr.ymax; j++) {
            auto wl = get_net_wirelength_estimate(net_id, bbptr);
            cong_matrix[i][j] += wl / double((bbptr.xmax - bbptr.xmin + 1)*(bbptr.ymax - bbptr.ymin + 1));
        }
    }
}
...
static void get_non_updateable_bb_old(ClusterNetId net_id, t_bb& bb_coord_new) {
    //TODO: account for multiple physical pin instances per logical pin
