    ...
    unsigned long long veriValue(const string& exp) {
        if (!isdigit(exp[0]))
            throw(std::invalid_argument(
                "Not a valid expression (i.e 4'h0A9 ) at line " + std::to_string(__LINE__) + " " + exp));
        stringstream ss(exp);
        string dd;
        char *str, *stops;
        unsigned int aa;
        ...
        if (aa < 1 || aa > 64)
            throw(std::invalid_argument(
                "Not supporting more than 64 bit values or less than one bit " + exp));
        if (dd[1] == 'b' || dd[1] == 'B')
            bb = 2;
        ...
        while (std::getline(ifs, line)) {
            ...
            if (tokens[0] == ".subckt") {
                ...
                if (name.find("dff") != std::string::npos) {
                    ...
                    if (init_tokens.size() < 3 || init_tokens[0] != ".param" || init_tokens[1] != "INIT_VALUE" || !is_binary_param_(init_tokens[2])) {
                        //
                        for (auto& t : tokens) {
                            ofs << t << " ";
                        }
                    ...
                    try {
                        ...
                        // yosys)
                        if (end(latch_lut_WIDTH_strs) == latch_lut_WIDTH_strs.find(tokens[1])) {
                            ofs << ln << "\n";
                            continue;
                        }
                        names_str names;
