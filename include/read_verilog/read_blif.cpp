...
public:
    BlifAllocCallback(e_circuit_format blif_format, AtomNetlist &main_netlist, const std::string netlist_id, const t_model *user_models, const t_model *library_models)
        : main_netlist_(main_netlist), netlist_id_(netlist_id), user_arch_models_(user_models), library_arch_models_(library_models), blif_format_(blif_format)
    {
        VTR_ASSERT(blif_format_ == e_circuit_format::BLIF || blif_format_ == e_circuit_format::EBLIF || blif_format_ == e_circuit_format::VERILOG);
    }
...
public: // Callback interface
    ...
    void names(std::vector<std::string> nets, std::vector<std::vector<blifparse::LogicValue>> so_cover) override
    {
        ...
        if (truth_table.empty() || (truth_table.size() == 1 && truth_table[0].size() == 1 && truth_table[0][0] == vtr::LogicValue::FALSE))
        {
                                  ...
                                  const t_model *library_models)
{
   ...
   FILE *infile = tmpfile();
   const char* in = fmemopen(argv[1], strlen(argv[1]), "r");
    simple_netlist n_l;
