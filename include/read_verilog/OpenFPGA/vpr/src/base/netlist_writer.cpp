...
public:
    Arc(std::string src_port,  ///< Source of the arc
        ...
        : source_name_(src_port), source_ipin_(src_ipin), sink_name_(snk_port), sink_ipin_(snk_ipin), delay_(del), condition_(cond)
    {
    }
...
public:                                                                 ///< Public methods
    LutInst(size_t lut_size,                                            ///< The LUT size
        ...
        : type_("LUT_K"), lut_size_(lut_size), lut_mask_(lut_mask), inst_name_(inst_name), port_conns_(port_conns), timing_arcs_(timing_arc_values), opts_(opts)
    {
    }
...
public:
    LatchInst(std::string inst_name,                                  ///< Name of this instance
        ...
        : instance_name_(inst_name), port_connections_(port_conns), type_(type), initial_value_(init_value), tcq_(tcq), tsu_(tsu), thld_(thld)
    {
    }
...
public:
    BlackBoxInst(std::string type_name,                                             ///< Instance type
                 ...
                 struct t_analysis_opts opts)
        : type_name_(type_name), inst_name_(inst_name), params_(params), attrs_(attrs), input_port_conns_(input_port_conns), output_port_conns_(output_port_conns), timing_arcs_(timing_arcs), ports_tsu_(ports_tsu), ports_thld_(ports_thld), ports_tcq_(ports_tcq), opts_(opts)
    {
    }
...
public:                                                  // Public interface
                        ...
                        PortType dir)
    {
        ...
        if (dir == PortType::INPUT)
        {
            ...
            {
                if (std::find(inputs_.begin(), inputs_.end(), matches[2]) == inputs_.end())
                { // Skip already existing multi-bit port names
                    io_name = matches[2];
                    portmap[matches[2]] = 0;
                }
                else
        ...
        else
        {
            ...
            {
                if (std::find(outputs_.begin(), outputs_.end(), matches[2]) == outputs_.end())
                { // Skip already existing multi-bit port names
                    portmap[matches[2]] = 0;
                    io_name = matches[2];
                }
...
void print_verilog_port(std::ostream &os, size_t &unconn_count, const std::string &port_name, const std::vector<std::string> &nets, PortType type, int depth, struct t_analysis_opts &opts)
{
    ...
    else
    {
        ...
        else
        {
            ...
            os << "{"
               << "\n";
            for (int ipin = (int)nets.size() - 1; ipin >= 0; --ipin)
            { // Reverse order to match endianess
                os << indent(depth + 1);
                if (nets[ipin].empty())
                {
                    // Disconnected
