 ...
 * @file
 *
 * This file implements the netlist writer which generates post-implementation
 * (i.e. post-routing) netlists in BLIF and Verilog formats, along with an SDF
 * file which can be used for timing back-annotation (e.g. for timing
 * simulation).
 *
 * The externally facing function from this module is netlist_writer() which
 * will generate the netlist and SDF output files.  netlist_writer() assumes
 * that a post-routing implementation is available and that there is a valid
 * timing graph with real delays annotated (these delays are used to create the
 * SDF).
 *
 * Implementation
 * ==============
 *
 * The netlist writer is primarily driven by the NetlistWriterVisitor class,
 * which walks the netlist using the NetlistWalker (see netlist_walker.h).  The
 * netlist walker calls NetlistWriterVisitor's  visit_atom_impl() method for
 * every atom (i.e. primitive circuit element) in VPR's internal data
 * structures.
 *
 * visit_atom_impl() then dispatches the atom to the appropriate
 * NetlistWriterVisitor member function (e.g. LUTs to make_lut_instance(),
 * Latches to make_latch_instance(), etc.).  Each of the make_*_instance()
 * functions records the external nets the atom connects to (see
 * make_inst_wire()) and constructs a concrete 'Instance' object to represent
 * the primitive. NetlistWriterVisitor saves these representations for later
 * output.
 *
 * 'Instance' is an abstract class representing objects which know how to create
 * thier own representation in BLIF, Verilog and SDF formats.  Most primitives
 * can be represented by the BlackBoxInst class, but special primitives like
 * LUTs and Latchs have thier own implementations (LutInst, LatchInst) to handle
 * some of their unique requirements.
 *
 * Once the entire netlist has been traversed the netlist walker will call
 * NetlistWriterVisitor's finish_impl() method which kicks off the generation of
 * the actual netlists and SDF files.  NetlistWriterVisitor's print_*() methods
 * setting up file-level global information (I/Os, net declarations etc.) and
 * then ask each Instance to print itself in the appropriate format to the
 * appropriate file.
 *
 * Name Escaping
 * =============
 * One of the challenges in generating netlists is producing consistent naming
 * of netlist elements. In particular valid BLIF names, Verilog names and SDF
 * names are not neccessarily the same. As a result we must escape invalid
 * characters/identifiers when producing some formats.
 *
 * All name escaping is handled at the output stage (i.e. in the print_*()
 * functions) since it is very format dependant.
 *
 * VPR stores names internally generally following BLIF conventions.  As a
 * result these names need to be escaped when generating Verilog or SDF.  This
 * is handled with escape_verilog_identifier() and escape_sdf_identifier()
 * functions.
 *
 * Primitives
 * ==========
 * Verilog netlist generation assumes the existance of appropriate primitives
 * for the various atom types (i.e. a LUT_K module, a DFF module etc.).  These
 * are currently defined the file
 * `<vtr>/vtr_flow/primitives.v`, where `<vtr>` is the root of the VTR source
 * tree.
 *
 * You will typically need to link with this file when attempting to simulate a
 * post-implementation netlist. Also see the comments in primitives.v for
 * important notes related to performing SDF back-annotated timing simulation.
 */
...
void print_blif_port(std::ostream &os, size_t &unconn_count,
                     const std::string &port_name,
                     const std::vector<std::string> &nets, int depth);
...
public:
   ...
   *              represent unconnected ports. To allow unique naming of these
   * nets unconn_count is used to uniquify these names. Whenever creating an
   * unconnected net (and using unconn_count to uniquify its name in the file)
   * unconn_count should be incremented.
   *
   *   @param depth  Current indentation depth.  This is used to figure-out how
   * much indentation should be applied. This is purely for cosmetic formatting.
   * Use indent() for generating consistent indentation.
   */
  virtual void print_blif(std::ostream &os, size_t &unconn_count,
                          int depth = 0) = 0;

  ///@brief Print the current instanse in Verilog, see print_blif() for argument
  /// descriptions
  virtual void print_verilog(std::ostream &os, size_t &unconn_count,
                             int depth = 0) = 0;

  ///@brief Print the current instanse in SDF, see print_blif() for argument
  /// descriptions
...
public: // Instance interface method implementations
  ...
  void print_blif(std::ostream &os, size_t &unconn_count, int depth) override {
    ...
    //
    //  We can write out the truth table as either a set of minterms (where the
    //  function is true), or a set of maxterms (where the function is false).
    //  We choose the representation which produces the fewest terms (and is
    //  smallest to represent in BLIF).
    ...
    for (size_t minterm = 0; minterm < lut_mask_.size(); minterm++) {
      if (lut_mask_[minterm] == output_value) {
        ...
        // Because BLIF puts the input values in order from LSB (left) to MSB
        // (right), we need to reverse the string
        std::reverse(bit_str.begin(), bit_str.end());

        // Trim to the LUT size
        std::string input_values(bit_str.begin(),
                                 bit_str.begin() + (lut_size_));
...
public:
  ...
  void print_blif(std::ostream &os, size_t & /*unconn_count*/,
...
public:
  ...
  void print_verilog(std::ostream &os, size_t &unconn_count,
                     int depth = 0) override {
    // Instance type
    std::string mod_name = type_name_;
    if (mod_name == "adder_carry") { // putting back adder_carry to upper case
                                     // for verilog only
...
public: // Public interface
  NetlistWriterVisitor(
      std::ostream &verilog_os, ///< Output stream for verilog netlist
      std::ostream &blif_os,    ///< Output stream for blif netlist
      std::ostream &sdf_os,     ///< Output stream for SDF
      std::shared_ptr<const AnalysisDelayCalculator> delay_calc,
      struct t_analysis_opts opts)
      : verilog_os_(verilog_os), blif_os_(blif_os), sdf_os_(sdf_os),
...
protected:
  ...
  void formating_encrypt_file(int state) {
    ...
    std::string attrs =
        ...
        "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDkRq28uVJ64yGZqOJrWPHJASe54e31P1"
        "ULNIw6\n"
        "wa9KiOcrrvH/rTq7wZ9xK3Y0xwmZjOfh/"
        "uf89+gwHBQWGMMMmZpPt6A3jR08oM0RmJpwOL4fAsFB\n"
        "kcgM7BOLt3gppbHQrZryRxXXkyFIoBSJAYTGuu4pfkOzNH7O0zUNwhfMzwIDAQAB\n"
        "`pragma protect version=\"1\"\n"
        "`pragma protect encrypt_agent=\"VCS\"\n"
        "`pragma protect encrypt_agent_info=\"S-2021.09-SP2 Build Date Feb 24 "
        "2022 20:42:21\"\n"
...
private: // Internal Helper functions
  ...
  void print_blif(int depth = 0) {
    ...
    for (const auto &kv : logical_net_sinks_) {
      ...
      for (auto &sink_wire_tnode_pair : kv.second) {
        blif_os_ << indent(depth) << ".names " << driver_wire << " "
                 << sink_wire_tnode_pair.first << "\n";
...
protected:
  ...
  std::shared_ptr<Instance> make_lut_instance(const t_pb *atom) {
    ...
    VTR_ASSERT(pb_graph_node->num_output_pins[0] ==
               1); // LUT has one output pin
...
public: // Public interface
  ...
  std::string make_io(const t_pb *atom, PortType dir) {
    ...
    if (top_pb_route.count(cluster_pin_idx)) {
      ...
      auto wire_name = make_inst_wire(atom_net_id, tnode_id, indexed_io_name,
                                      wire_dir, 0, 0);

      // Connect the wires to to I/Os with assign statements
      if (wire_dir == PortType::INPUT) {
        assignments_.emplace_back(indexed_io_name,
                                  escape_verilog_identifier(wire_name));
      } else {
        assignments_.emplace_back(escape_verilog_identifier(wire_name),
                                  indexed_io_name);
      }
    }
  ...
  void print_primary_io(int depth) {
    ...
    for (auto iter = inputs_.begin(); iter != inputs_.end(); ++iter) {
      ...
      std::string range;
      if (portmap[*iter] > 0)
        verilog_os_ << indent(depth + 1) << "input [" << portmap[*iter]
                    << ":0] " << *iter;
      else
    ...
    for (auto iter = outputs_.begin(); iter != outputs_.end(); ++iter) {
      std::string range;
      if (portmap[*iter] > 0)
        verilog_os_ << indent(depth + 1) << "output [" << portmap[*iter]
                    << ":0] " << *iter;
      else
...
void netlist_writer(const std::string basename,
  ...
  std::stringstream encrypted_verilog_ss; // Create a stringstream to store the
                                          // encrypted Verilog data
  if (!isNestEncrypted) {
  ...
  if (isNestEncrypted) {
    ...
    NetlistWriterVisitor visitor(encrypted_verilog_ss, blif_os_e, sdf_os_e,
                                 delay_calc, opts);

    NetlistWalker nl_walker(visitor);

    nl_walker.walk();
    const char *out_buf = file_name.c_str();
    // saving name in hdl_encrypt settings
    char *out_file_name = Verific::Strings::save(out_buf, "");
    Verific::hdl_encrypt::RegisterFlexStreamCallBack(get_verific_stream);
    Verific::hdl_encrypt::EncryptVerilogFile(encrypted_verilog_ss.str().c_str(),
                                             out_file_name, &ieee_1735);

    VTR_LOG("Writing Encrypted Verilog: %s\n", file_name.c_str());

    // std::filesystem::path current_directory =
    // std::filesystem::current_path(); std::filesystem::path
    // generated_file_path = current_directory / out_file_name;
