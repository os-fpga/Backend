...
void WriterVisitor::printLib(int depth) {
  uint16_t tr = ltrace();
  if (tr >= 2)
    lprintf("  WriterVisitor::printLib( depth= %i )\n", depth);
  ...
  for (const auto& inst : cell_instances_) {
    tname = inst->get_type_name();
    if (tr >= 4) {
      lprintf("  tname %s\n", tname.c_str());
    }
    if (not written_cells.count(tname)) {
      inst->printLib(lib_writer, lib_os_);
  ...
  if (tr >= 3) {
    lprintf("--WriterVisitor::printLib: written %zu lcells\n", written_cells.size());
    if (tr >= 4) {
...
Cell* WriterVisitor::make_lut_instance(const t_pb* atom) {
  ...
  auto& ls = lout();
  if (tr >= 4) {
    ls << "make_lut_instance() lut_size= " << lut_size << endl;
  }
  ...
  auto inst = new LutInst(lut_size, lut_mask, inst_name, port_conns, timing_arcs, opts_);
