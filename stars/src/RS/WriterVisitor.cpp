...
void WriterVisitor::print_verilog(int depth) {
  uint16_t tr = ltrace();
  if (tr >= 2)
    lprintf("  WVisitor::print_verilog( depth= %i )\n", depth);
...
void WriterVisitor::print_lib(int depth) {
  uint16_t tr = ltrace();
  if (tr >= 2)
    lprintf("  WriterVisitor::print_lib( depth= %i )\n", depth);
...
void WriterVisitor::print_sdf(int depth) {
...
std::shared_ptr<Instance> WriterVisitor::make_lut_instance(const t_pb* atom) {
  assert(atom);
  const t_pb& atm = *atom;
...
std::shared_ptr<Instance> WriterVisitor::make_latch_instance(const t_pb* atom) {
 ...
 */
std::shared_ptr<Instance> WriterVisitor::make_ram_instance(const t_pb* atom) {
  const auto& top_pb_route = find_top_pb_route(atom);
  const t_pb_graph_node* pb_graph_node = atom->pb_graph_node;
...
std::shared_ptr<Instance> WriterVisitor::make_multiply_instance(const t_pb* atom) {
...
std::shared_ptr<Instance> WriterVisitor::make_adder_instance(const t_pb* atom) {
  auto& timing_ctx = g_vpr_ctx.timing();
...
std::shared_ptr<Instance> WriterVisitor::make_blackbox_instance(const t_pb* atom) {
  const auto& top_pb_route = find_top_pb_route(atom);
  const t_pb_graph_node* pb_graph_node = atom->pb_graph_node;
  const t_pb_type* pb_type = pb_graph_node->pb_type;
