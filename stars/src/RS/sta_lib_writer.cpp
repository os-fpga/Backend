...
void sta_lib_writer::write_header(std::ostream& os) {
  uint16_t tr = ltrace();
  if (tr >= 2) lputs("sta_lib_writer::write_header()");
...
void sta_lib_writer::write_cell(std::ostream& os, const lib_cell& cell) {
  lout() << "STARS: Writing cell <" << cell.name() << ">..." << endl;
...
void sta_lib_writer::write_timing_relation(std::ostream& os, const TimingArc& timing) {
  lib_pin related_pin = timing.related_pin();
  string related_pin_nm = related_pin.name();

  uint16_t tr = ltrace();
  if (tr >= 4) {
    lprintf("  LW::write_timing_relation()  related_pin= %s\n", related_pin_nm.c_str());
  }
