...
bool rsEnv::getPidExePath(int pid, std::string& out) noexcept
{
    return true;
}
...
namespace {
template <typename T>
inline void lst(const char* nm, T t) { cout << std::setw(30) << nm << " : " << t << endl; }
}

void rsEnv::listDevEnv() const noexcept
{
    cout << "\n\t   RSBE ver.  " << shortVer_ << endl;
    printf("\t compiled:  %s\n\n", compTimeCS());
