...
inline constexpr int64_t add64(int a, int b) noexcept { return int64_t(a) + int64_t(b); }
inline constexpr int64_t sub64(int a, int b) noexcept { return int64_t(a) - int64_t(b); }

inline constexpr int protectedRound(double x) noexcept {
    if (x >= 0) {
        if (x >= double(INT_MAX) - 0.5)
            return INT_MAX;
        return x + 0.5;
    }
    if (x <= double(INT_MIN) + 0.5)
        return INT_MIN;
    return x - 0.5;
}

inline constexpr int inlineRound(double x) noexcept { return x >= 0 ? x + 0.5 : x - 0.5; }

inline constexpr int64_t inlineRound64(double x) noexcept { return x >= 0 ? int64_t(x + 0.5) : int64_t(x - 0.5); }
...
struct Iv
{
    ...
    void set(int a, int b) noexcept { a_ = a; b_ = b; }
    void setZero() noexcept { a_ = 0; b_ = 0; }
    ...
    void normalize() noexcept { if (a_ > b_) std::swap(a_, b_); }
    ...
    void bloat(int delta) noexcept { assert(delta >= 0); bloat(delta, delta); }
    ...
    bool intersects(Iv iv) const noexcept { return intersects(a_, b_, iv.a_, iv.b_); }
    ...
    inline static int ovLen(int a, int b, int c, int d, int* p1=nullptr, int* p2=nullptr) noexcept;
    int ovLen(Iv iv, int* p1=nullptr, int* p2=nullptr) const noexcept { return ovLen(a_, b_, iv.a_, iv.b_, p1, p2); }
    ...
    bool covers(Iv iv) const noexcept {
        assert(valid() && normal());
        assert(iv.valid());
        return insideNorm(iv.a_, a_, b_) && insideNorm(iv.b_, a_, b_);
    }
    bool strictlyCovers(Iv iv) const noexcept {
        assert(valid() && normal());
        assert(iv.valid());
        return insideOpenNorm(iv.a_, a_, b_) && insideOpenNorm(iv.b_, a_, b_);
    }
