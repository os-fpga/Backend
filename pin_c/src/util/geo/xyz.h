...
struct XY
{
    ...
    void setPoint(int a, int b) noexcept { x_ = a; y_ = b; }
    void setPoint(XY a) noexcept { *this = a; }
    void setZero() noexcept { x_ = 0; y_ = 0; }

    void moveBy(int dx, int dy) noexcept { x_ += dx; y_ += dy; }
    void moveBy(XY d) noexcept { moveBy(d.x_, d.y_); }

    void negate() noexcept { x_ = -x_; y_ = -y_; }
    XY negated() const noexcept { return XY(-x_, -y_); }

    int64_t dotP(XY b) const noexcept   { return int64_t(x_) * int64_t(b.x_) + int64_t(y_) * int64_t(b.y_); }
    int64_t crossP(XY b) const noexcept { return int64_t(x_) * int64_t(b.y_) - int64_t(y_) * int64_t(b.x_); }
    ...
    void operator += (XY p) noexcept { x_ += p.x_; y_ += p.y_; }
    void operator -= (XY p) noexcept { x_ -= p.x_; y_ -= p.y_; }

    void multiply(double t) noexcept { x_ = protectedRound(t * x_); y_ = protectedRound(t * y_); }
    XY multipliedBy(double t) const noexcept { XY c(*this); c.multiply(t); return c; }
    XY& operator *= (double t) noexcept { multiply(t); return *this; }

    int distL1(XY p) const noexcept { return std::abs(x_ - p.x_) + std::abs(y_ - p.y_); }
    int dist(XY p) const noexcept   { return std::abs(x_ - p.x_) + std::abs(y_ - p.y_); }
    int dist(int a, int b) const noexcept { return std::abs(x_ - a) + std::abs(y_ - b); }
    int distL2(XY p) const noexcept { return protectedRound(::hypot(p.x_ - x_, p.y_ - y_)); }
    double sqDist(XY p) const noexcept { return double(p.x_ - x_) * double(p.x_ - x_) + double(p.y_ - y_) * double(p.y_ - y_); }
...
struct XYZ : public XY
{
    ...
    void set3(int a, int b, int c) noexcept { x_ = a; y_ = b; z_ = c; }
    void set3(XY p, int c) noexcept { x_ = p.x_; y_ = p.y_; z_ = c; }
    void set3(XYZ u) noexcept { *this = u; }

    bool operator == (XYZ u) const noexcept { return x_ == u.x_ && y_ == u.y_ && z_ == u.z_; }
    ...
    bool equals(int a, int b, int c) const noexcept { return x_ == a && y_ == b && z_ == c; }
...
inline XY operator - (XY a, XY b) noexcept { return XY(a.x_ - b.x_, a.y_ - b.y_); }
inline XY operator + (XY a, XY b) noexcept { return XY(a.x_ + b.x_, a.y_ + b.y_); }
