...
bool PcfReader::read_pcf(const string& f)
{
    ...
    string line; bool has_error = false;
    ...
    while (std::getline(infile, line)) {
        ...
        set_io_cmd.clear(); user_pin.clear(); bump_pin.clear(); dash_mode.clear(); mode_name.clear();
        optional_dash_internal_pin.clear(); optional_internal_pin.clear();
        ...
        if (has_internal_pin) {
            if (!(iss >> set_io_cmd >> user_pin >> bump_pin >> dash_mode >> mode_name >> optional_dash_internal_pin >> optional_internal_pin)) {
                has_error = true;
                break; // error
            }
        } else {
            if (!(iss >> set_io_cmd >> user_pin >> bump_pin >> dash_mode >> mode_name)) {
                has_error = true;
                break; // error
            }
        }
