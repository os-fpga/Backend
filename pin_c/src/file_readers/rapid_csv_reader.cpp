...
bool rapidCsvReader::read_csv(const string& fn, bool check)
{
    ...
    rapidcsv::Document doc( fn,
        ...
        rapidcsv::ConverterParams(true /*user default number for non-numberical strings*/, 0.0 /* default float*/,
                                    -1 /*default integer*/), rapidcsv::LineReaderParams()
    ...
    for (uint i = 0; i < num_rows; i++)
        io_tile_pin_xyz_[i].x_ = tmp[i];

    tmp = doc.GetColumn<int>("IO_tile_pin_y");
    assert(tmp.size() == num_rows);
    for (uint i = 0; i < num_rows; i++)
        io_tile_pin_xyz_[i].y_ = tmp[i];

    tmp = doc.GetColumn<int>("IO_tile_pin_z");
    assert(tmp.size() == num_rows);
    for (uint i = 0; i < num_rows; i++)
        io_tile_pin_xyz_[i].z_ = tmp[i];

    assert( bump_pin_name_.size() == io_tile_pin_.size() && io_tile_pin_.size() == num_rows );
...
XYZ rapidCsvReader::get_pin_xyz_by_bump_name(const string& mode,
                                           ...
                                           const string& gbox_pin_name) const
{
    ...
    for (uint i = 0; i < num_rows; i++) {
        if ((bump_pin_name_[i] == bump_name) && (mode_vector[i] == "Y")) {
            if (gbox_pin_name.length() == 0 || ((gbox_pin_name.length() > 0) && (gbox_name_[i] == gbox_pin_name))) {
