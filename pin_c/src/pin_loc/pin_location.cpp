...
bool pin_location::reader_and_writer()
{
    ...
    if (tr >= 2) {
        ls << "\t usage_requirement_0 : " << boolalpha << usage_requirement_0 << endl;
...
bool pin_location::read_user_design()
{
    uint16_t tr = ltrace();
    if (tr >= 2)
        lprintf("pin_location::read_user_design() __ getting port info from blif/json\n");
    ...
    if (json_ifs.is_open()) {
        ...
        user_design_inputs_ = std::move(rd_blif.inputs_);
        user_design_outputs_ = std::move(rd_blif.outputs_);
    }

    if (tr >= 2) {
        lprintf("DONE read_user_design()  user_design_inputs_.size()= %u  user_design_outputs_.size()= %u\n",
...
bool pin_location::read_user_pinloc_constrain_file()
{
    uint16_t tr = ltrace();
    if (tr >= 2)
        lprintf("pin_location::read_user_pinloc_constrain_file() __ Reading user place constrain\n");
...
bool pin_location::create_place_file(rapidCsvReader& rs_csv_reader)
{
    ...
    if (tr >= 2) {
        ls << "pin_location::create_place_file() __ Creating place file\n"
              "  cl_.get_param(--output) : " << out_fn << endl;
    }
    ...
    // parse each constraint line in constrain file, generate place location accordingly
    //
    ...
    for (const vector<string>& pcf_cmd : pcf_pin_cmds_) {
        ...
        if (pcf_cmd[0] != "set_io" && pcf_cmd[0] != "set_clk")
            continue;
        gbox_pin_name.clear(); search_name.clear();
        ...
        bool is_out_pin = is_in_pin ? false : vec_contains(user_design_outputs_, user_design_pin_name);
        ...
        // use (device_pin_name + " " + gbox_pin_name) as search name to check overlap pin in constraint
        ...
        XYZ xyz = rs_csv_reader.get_pin_xyz_by_bump_name(mode, device_pin_name, gbox_pin_name);
        assert(xyz.valid());
...
bool pin_location::read_csv_file(rapidCsvReader& rs_csv_rd)
{
    uint16_t tr = ltrace();
    if (tr >= 2)
        lputs("pin_location::read_csv_file() __ Reading new csv");
    ...
    PortDirection port_direction)
{
    ...
    for (uint i = rs_csv_rd.start_position_; i < rs_csv_rd.bump_pin_name_.size(); i++) {
        ...
        for (uint j = 0; j < rs_csv_rd.mode_names_.size(); j++) {
          ...
          if (port_direction == INPUT) {
            if (is_input_mode(mode_name)) {
              for (uint k = rs_csv_rd.start_position_;
                   k < mode_data.size(); k++) {
                if ((mode_data[k] == "Y") &&
...
static bool read_json_ports(const nlohmann::ordered_json& from, vector<pin_location::PortInfo>& ports)
{
    ports.clear();
                ...
                vector<string>& inputs, vector<string>& outputs)
{
    ...
    if (tr >= 2) {
        ls << " rootObj.size() = " << root_sz
           << "  rootObj.is_array() : " << std::boolalpha << rootObj.is_array() << endl;
    }
    ...
    if (tr >= 2)
        ls << " got " << inputs.size() << " inputs and " << outputs.size() << " outputs" << endl;
