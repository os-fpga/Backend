...
namespace pinc
{
  ...
  static string USAGE_MSG_3 =
      "usage options: set_clock_pin -device_clock PHYSICAL_CLOCK "
      "- design_clock USER_DESIGN_CLOCK "; // for rs internall,
  ...
  */
  ...
  bool pin_location::logical_clocks_to_GEMINI_clks()
  {
    ...
    bool d_c = false;
    bool p_c = false;
    std::ofstream file("fpga_repack_constraints.xml");
    file << "<repack_design_constraints>" << std::endl;
    for (int i = 0; i < set_clks.size(); i++)
    {
